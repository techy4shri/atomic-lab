...
export class BondLines {
  ...
  updateFromBonds(bonds: Bonds, pos: Float32Array) {
    ...
    bonds.forEach((i,j)=>{
      ...
      const ia = 3*i, jb = 3*j;
      this.positions[idx*3+0] = pos[ia];
      this.positions[idx*3+1] = pos[ia+1];
      this.positions[idx*3+2] = pos[ia+2];
      this.positions[(idx+1)*3+0] = pos[jb];
      this.positions[(idx+1)*3+1] = pos[jb+1];
      this.positions[(idx+1)*3+2] = pos[jb+2];
      idx += 2;
    });
